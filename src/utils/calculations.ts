...
export function evaluateStoreSignal(input: StoreSignalInput): StoreSignalResult {
  const belowAll = input.price < input.ma20 && input.price < input.ma50 && input.price < input.ma100 && input.price < input.ma200;
  const longTermUpButPriceCollapsed = belowAll && input.ma200 < input.ma100 && input.ma100 < input.ma50 && input.ma50 < input.ma20;
  const downtrend = input.price < input.ma20 && input.ma20 < input.ma50 && input.ma50 < input.ma100 && input.ma100 < input.ma200;
  const crossedAbove20 = input.prevPrice < input.prevMa20 && input.price >= input.ma20;
  const reboundSignal = crossedAbove20 && input.price < input.ma50 && input.price < input.ma100 && input.price < input.ma200;
  const labels: string[] = [];
  ...
  const isCandidate = belowAll || longTermUpButPriceCollapsed || downtrend || reboundSignal;
...
export function validateInputs(settings: StrategySettings, cycle: CycleInput): string[] {
  const errors: string[] = [];
  if (settings.gValue <= 0) errors.push("G값은 0보다 커야 합니다.");
  if (settings.bandRate <= 0 || settings.bandRate >= 1) errors.push("밴드 비율은 0보다 크고 1보다 작아야 합니다.");
  if (settings.cyclePoolUseLimit <= 0 || settings.cyclePoolUseLimit > 1) errors.push("Pool 사용 한도는 0보다 크고 1보다 작거나 같아야 합니다.");
  if (cycle.shares <= 0) errors.push("보유 수량은 0보다 커야 합니다.");
  if (settings.orderUnit < 1) errors.push("주문 단위는 1 이상이어야 합니다.");
  if (cycle.storeInjection > cycle.currentStore) errors.push("STORE 투입금은 현재 STORE보다 클 수 없습니다.");
  return errors;
}
