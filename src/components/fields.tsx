...
export function NumberField({ label, value, onChange, step = 1, min, suffix }: NumberFieldProps) {
  ...
  return (
    <label className="field">
      ...
      <div className="input-wrap">
        <input type="number" value={value} min={min} step={step} onChange={handleChange} />
        {suffix && <em>{suffix}</em>}
      </div>
    </label>
  );
